...
export async function initLoader() {
  ...
  if (!fs.existsSync(pluginsDir)) {
    logger.warn('LOADER', 'Plugins directory not found')
    return
  }
    ...
    for (const file of files) {
      try {
        ...
        if (cmd && cmd.name) {
          ...
          totalCmds++
          logger.pluginLoaded(cmd.name, catInfo.name, 1)
        }
      } catch (e) {
        logger.error('LOADER', `Error loading plugin ${file}`, e.message)
      }
    }
  }
  logger.success('LOADER', `Total ${totalCmds} plugins loaded successfully`)
}