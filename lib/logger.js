...
export const logger = {
  ...
  incoming (from, sender, body, mediaType, jidType, isCmd) {
    const ts = timestamp()
    const badgeMsg = badge('MSG', C.black, C.bgYellow)
    ...
    const line = `${ts} ${badgeMsg} ${jidBadge} ${C.cyan}${sender}${C.reset} ${C.gray}→${C.reset} ${C.white}${from}${C.reset} ${mediaBadge} ${cmdBadge} ${C.white}${body}${C.reset}`
    console.log(line)
  },
