...
export const logger = {
  ...
  incoming(from, sender, body, mediaType = 'TEXT', jidType = 'DM', isCmd = false) {
    const ts = timestamp()
    const msgBadge = badge('MSG', C.black, C.bgYellow)
    const jidBadge = `${C.yellow}[${jidType}]${C.reset}`
    const senderStr = `${C.cyan}${sender}${C.reset}`
    const fromStr = `${C.white}${from}${C.reset}`
    const mediaBadge = `${C.magenta}[${mediaType}]${C.reset}`
    const cmdBadge = isCmd ? `${C.cyan}[CMD]${C.reset}` : ''
    const bodyStr = `${C.white}${body}${C.reset}`
    
    console.log(`${ts} ${msgBadge} ${jidBadge} ${senderStr} → ${fromStr} ${mediaBadge} ${cmdBadge} ${bodyStr}`)
  },
  ...
  pluginLoaded (name, type, cmdCount) {
    console.log(`${timestamp()} ${badge('LOAD', C.white, C.bgCyan)} ${C.cyan}[${type}]${C.reset} ${C.white}${name}${C.reset} ${C.gray}(${cmdCount} registered)${C.reset}`)
  },

  executed (cmd, sender, success = true) {
    const icon = success ? '✅' : '❌'
    console.log(`${timestamp()} ${badge('CMD', C.white, C.bgCyan)} ${icon} ${C.bold}${cmd}${C.reset} ${C.gray}by${C.reset} ${C.cyan}${sender}${C.reset}`)
  },

  ramStats () {
    const mem = process.memoryUsage()
    const toMB = b => (b / 1024 / 1024).toFixed(2)
    console.log(`${timestamp()} ${badge('RAM', C.white, C.bgBlue)} RSS: ${C.yellow}${toMB(mem.rss)}MB${C.reset} | Heap: ${C.yellow}${toMB(mem.heapUsed)}/${toMB(mem.heapTotal)}MB${C.reset}`)
  }
}
